...
//    public native Config withHostMemory(MemoryCreator memCreator);
    ...
    /// Configures whether the engine will use copy-on-write memory initialization
    /// for WebAssembly modules.
    ///
    /// On Linux this feature uses `memfd_create` and file seals, which require
    /// the `MEMFD_CREATE` syscall to be available. In some low-privilege
    /// environments (certain container runtimes, seccomp-filtered processes, or
    /// kernels older than 3.17) this syscall is blocked, causing a
    /// "cannot add seals to the memfd" error at module instantiation time.
    /// Set this to `false` to fall back to ordinary memory initialization and
    /// avoid the memfd usage entirely.
    ///
    /// This is `true` by default.
    public native Config memoryInitCow(boolean enable);
